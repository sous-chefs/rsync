...
describe file('/etc/rsyncd.conf') do
  its('content') { should match %r{path = /data/repos/prod/centos} } # rsync added path
  ...
  its('content')  { should match %r{hosts allow = 127.0.0.1, 10.4.1.0/24, 192.168.4.0/24} } # rsync networking is setup
  its('content')  { should match %r{hosts deny = 0.0.0.0/0} } # rsync networking is setup
  ...
  its('content')  { should match %r{log file = /tmp/centos-sync} } # rsync networking is setup
  its('content')  { should match %r{pre-xfer exec = /bin/true} } # rsync networking is setup
  its('content')  { should match %r{post-xfer exec = /bin/true} } # rsync networking is setup
